...
When **TTS_AFTER_TASK** is enabled in `.env` and **OPENAI_API_KEY** is set, the orchestrator plays a beep then speaks a short summary after each completed task. The text is generated by an LLM from a template, then spoken via OpenAI TTS.

- **`voice-summary.md`** — Prompt template for the spoken summary. Placeholders are substituted by the pipeline (e.g. `${AGENT_ID}`, `${TASK_ID}`, `${TASK_DESCRIPTION}`, `${TONE}`, `${MILESTONE}`, `${ITERATION}`, `${CONFIDENCE}`, `${VERDICT}`); the sections **IMPLEMENTER_SUMMARY** and **VALIDATOR_SUMMARY** are filled from the implementer and validator output. Customize this file to change tone and structure (e.g. “Hello sir! ${AGENT_ID} has completed a task…”). Override with `TTS_SUMMARY_PROMPT_FILE`; set `TTS_TONE`, `TTS_SUMMARY_MODEL`, `TTS_MODEL`, `TTS_VOICE`, `TTS_SPEED` in `.env`. See `docs/ENV-README.md`.
