...
1. **Metadata (required at top):** Include a plan-an-go metadata block as the first content, wrapped in HTML comments so previews do not render it. Use current date/time in ISO 8601 and set `generated_cli` to this assistant (e.g. `claude`):

<!-- 
```plan_meta_data
{"created_by":"generate-prd","created_at":"<ISO 8601>","last_updated":"<ISO 8601>","generated_cli":"claude"}
```
-->

2. **Title:** `# PRD — <Title>`
3. **Overview:** 2–4 sentences (what, who, why).
4. **Goals:** Bullet list of one-line outcomes.
5. **Non-goals:** Explicit out-of-scope items.
6. **User personas / stakeholders:** Primary and optional secondary.
7. **Requirements:** Functional (F1, F2, …) and non-functional (NF1, NF2, …).
8. **Success criteria:** Testable conditions that define "done".
9. **Notes / assumptions / risks:** Optional.
