...
- **Use mock data first:** Implementation must support a fast path driven by **mock data** (multiple `.json` or `.jsonl` files) for development, seeding, and E2E tests—e.g. `mock/todos.json` or `mock/todos.jsonl`. The plan must include tasks to create these files and wire the app (or seed script) to load from them when appropriate (e.g. env flag or test fixtures).
