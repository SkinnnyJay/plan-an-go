#!/usr/bin/env bash
# Run the journal example: generate PLAN from PRD if needed, then run implementer loop.
# Run from repo root: ./examples/journal/run.sh
...
EXAMPLES_DIR="examples/journal"
OUT_DIR="$ROOT/$EXAMPLES_DIR"
TMP_DIR="${PLAN_AN_GO_TMP:-$ROOT/tmp}"
mkdir -p "$TMP_DIR"
OUT_LOG="${TMP_DIR}/example-journal-run-$(date +%Y%m%d-%H%M%S).log"

if [ ! -f "$OUT_DIR/PLAN.md" ]; then
  echo "Generating PLAN.md from PRD..."
  PLANNER_OUT=$(mktemp "$TMP_DIR/journal-planner-out.XXXXXX")
  PLANNER_ERR=$(mktemp "$TMP_DIR/journal-planner-err.XXXXXX")
