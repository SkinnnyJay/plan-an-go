...
- **Use mock data first:** Implementation must support a fast path driven by **mock data** (multiple `.json` or `.jsonl` files) for development, seeding, and E2E tests—e.g. `mock/vacation-todos.json`, `mock/listings.json`. The plan must include tasks to create these files and wire the app to load from them when appropriate (e.g. `USE_MOCK_DATA=true` or test fixtures), so that Airbnb API calls are optional and mock data is the default for speed.
- **DB and APIs as optional/secondary:** Prisma/SQLite and Airbnb API integration can be used when not in mock mode; the plan should make it clear that mock data is the default for speed and that DB/API are used only when mock data is disabled.
