...
- **Use mock data first:** Implementation must support a fast path driven by **mock data** (multiple `.json` or `.jsonl` files) for development, seeding, and E2E tests—e.g. `mock/search-results.json`, `mock/videos.jsonl`. The plan must include tasks to create these files and wire the app (e.g. API route or proxy) to serve from them when appropriate (e.g. `USE_MOCK_DATA=true` or no API key), so that YouTube API calls are optional and mock data is the default for speed.
- **DB and YouTube API as optional/secondary:** Persistence (SQLite/localStorage) and YouTube Data API v3 / IFrame API can be used when not in mock mode; the plan should make it clear that mock data is the default for speed and that live API/DB are used only when mock data is disabled.
