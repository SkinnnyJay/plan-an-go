...
if ! command -v fswatch &> /dev/null; then
    ...
    echo -e "${YELLOW}Install it with: brew install fswatch${RESET}"
    exit 1
fi
...
# Create temp file to track seen files (bash 3.2 compatible)
STATE_FILE=$(mktemp /tmp/plan-an-go-file-watch.XXXXXX)
...
process_event() {
    ...
    # Get relative path
    local rel_path=$(get_relative_path "$event_path")
...
# -e: exclude patterns (git, node_modules, etc.)
fswatch -r -m poll_monitor \
    ...
    if [ ! -e "$event_path" ]; then
        ...
        # File exists and we've seen it before - modified
        process_event "$event_path" "Modified"
    else
        # File exists but we haven't seen it - created
