#!/usr/bin/env bash
# Large test: PRD-VACATION-AIRBNB.md artifact exists and optionally run planner to produce a PLAN.
# Uses __tests__/artifacts/PRD-VACATION-AIRBNB.md. Writes only to ./tmp/.
...
PRD="$ARTIFACTS/PRD-VACATION-AIRBNB.md"
OUT_PLAN="./tmp/plan-vacation-airbnb.md"
VERBOSE=""
[ " ${*:-}" = " --verbose" ] && VERBOSE=1
...
[ -n "$VERBOSE" ] && echo "  OK: PRD-VACATION-AIRBNB.md present and valid"
...
"$PLANNER" --in "$PRD" --out "$OUT_PLAN" >> ./tmp/prd-vacation-airbnb.large.out 2>&1
exitcode=$?
if [ "$exitcode" -ne 0 ]; then
  echo "Planner failed (exit $exitcode); check CLI and API key" >&2
  cat ./tmp/prd-vacation-airbnb.large.out >&2
  exit 1
fi
