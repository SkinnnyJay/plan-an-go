#!/usr/bin/env bash
# Large test: PRD-YOUTUBE-CLONE.md artifact exists and optionally run planner to produce a PLAN.
# Uses __tests__/artifacts/PRD-YOUTUBE-CLONE.md. Writes only to ./tmp/.
...
PRD="$ARTIFACTS/PRD-YOUTUBE-CLONE.md"
OUT_PLAN="./tmp/plan-youtube-clone.md"
VERBOSE=""
[ " ${*:-}" = " --verbose" ] && VERBOSE=1
...
[ -n "$VERBOSE" ] && echo "  OK: PRD-YOUTUBE-CLONE.md present and valid"
...
"$PLANNER" --in "$PRD" --out "$OUT_PLAN" >> ./tmp/prd-youtube-clone.large.out 2>&1
exitcode=$?
if [ "$exitcode" -ne 0 ]; then
  echo "Planner failed (exit $exitcode); check CLI and API key" >&2
  cat ./tmp/prd-youtube-clone.large.out >&2
  exit 1
fi
