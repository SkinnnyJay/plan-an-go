#!/usr/bin/env bash
# Large test: PRD-JOURNAL.md artifact exists and optionally run planner to produce a PLAN.
# Uses __tests__/artifacts/PRD-JOURNAL.md. Writes only to ./tmp/.
...
PRD="$ARTIFACTS/PRD-JOURNAL.md"
OUT_PLAN="./tmp/plan-journal.md"
VERBOSE=""
[ " ${*:-}" = " --verbose" ] && VERBOSE=1
...
[ -n "$VERBOSE" ] && echo "  OK: PRD-JOURNAL.md present and valid"
...
"$PLANNER" --in "$PRD" --out "$OUT_PLAN" >> ./tmp/prd-journal.large.out 2>&1
exitcode=$?
if [ "$exitcode" -ne 0 ]; then
  echo "Planner failed (exit $exitcode); check CLI and API key" >&2
  cat ./tmp/prd-journal.large.out >&2
  exit 1
fi
