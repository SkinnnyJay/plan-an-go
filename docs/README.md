...
| [**SIMULATED-4-CYCLE-OUTPUT.md**](SIMULATED-4-CYCLE-OUTPUT.md) | **Understanding pipeline output** — Annotated 4-cycle run (header, one iteration in detail, flow at a glance, full raw output). How to read the log; 2 concurrent agents (pool vs `--wait-for-all`). |
